...
def add_symbol_def(doc, entries):
    """Add symbol definitions with 'ただし、' prefix and inline math images.

    Args:
        doc:     Document object.
        entries: list of (latex_symbol_str, definition_str) tuples.
    """
    if not entries:
        return

    # "ただし、" prefix paragraph
    p_pre = doc.add_paragraph()
    p_pre.paragraph_format.space_before = Pt(2)
    p_pre.paragraph_format.space_after = Pt(0)
    p_pre.paragraph_format.left_indent = Cm(0.5)
    run_pre = p_pre.add_run("ただし、")
    set_run_font(run_pre, size=10)

    for sym, defn in entries:
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(0)
        p.paragraph_format.space_after = Pt(1)
        p.paragraph_format.left_indent = Cm(1.5)

        # Render symbol as small inline math image
        sym_img = render_math(sym, fontsize=11)
        run_sym = p.add_run()
        run_sym.add_picture(sym_img, height=Inches(0.18))

        # Colon and definition text
        run_def = p.add_run(f" ：{defn}")
        set_run_font(run_def, size=10)
...
def add_section_target_generator(doc):
    ...
    add_symbol_def(doc, [
        (r"\mathbf{x}_{gt}", "目標の状態ベクトル（9次元）"),
        ("x, y, z", "位置（東・北・高度）[m]"),
        ("v_x, v_y, v_z", "速度成分 [m/s]"),
        ("a_x, a_y, a_z", "加速度成分 [m/s²]"),
        (r"(\cdot)^T", "転置（列ベクトルを表す）"),
    ])
    ...
    add_symbol_def(doc, [
        ...
        (r"f(\mathbf{x})", "運動方程式（重力＋大気抵抗）"),
        (r"\mathbf{x}_n", "タイムステップ n における状態ベクトル"),
        (r"\mathbf{x}_{n+1}", "次ステップの状態ベクトル"),
        (r"\Delta t", "積分タイムステップ [s]"),
    ])
    ...
    add_symbol_def(doc, [
        (r"\dot{x}, \dot{y}, \dot{z}", "位置の時間微分（速度成分）[m/s]"),
        ("v_x, v_y, v_z", "速度成分 [m/s]"),
        (r"\dot{v}_x, \dot{v}_y, \dot{v}_z", "速度の時間微分（加速度成分）[m/s²]"),
        ...
        (r"\rho(z)", "高度 z における大気密度 [kg/m³]"),
        (r"\rho_0", "海面大気密度（= 1.225 kg/m³）"),
        ("H", "大気スケール高度 [m]"),
    ])
    ...
    add_math_image(doc,
        ...
        fontsize=14)
    add_symbol_def(doc, [
        (r"\mathbf{F}_{drag}", "抗力ベクトル（速度逆方向）[N]"),
        (r"\rho(z)", "高度依存大気密度 [kg/m³]"),
        ("v", "速度の大きさ [m/s]"),
        ("C_D", "抗力係数 [-]"),
        ("A", "代表断面積 [m²]"),
        (r"\hat{v}", "速度方向の単位ベクトル"),
        ("L", "揚力の大きさ [N]"),
        ("C_L", "揚力係数（= C_D × ε_LD）[-]"),
        (r"\varepsilon_{LD}", "揚抗比（Lift-to-Drag ratio）[-]"),
        (r"\hat{n}_{up}", "鉛直面内の上向き単位ベクトル"),
        (r"\hat{z}", "鉛直（高度）方向の単位ベクトル"),
        (r"\hat{n}_{lat}", "横方向単位ベクトル（速度と上向きの外積方向）"),
        (r"\mathbf{F}_{lift}", "揚力ベクトル [N]"),
        (r"\sigma", "バンク角（機体ロール角）[rad]"),
    ])
...
def add_section_radar_simulator(doc):
    ...
    add_symbol_def(doc, [
        ...
        ("SNR_{ref}", "基準距離における平均SNR [dB]"),
        ("r", "目標までのスラントレンジ [m]"),
        (r"\log_{10}", "常用対数"),
        (r"\gamma_T", "CFAR検出閾値（= −ln P_FA）"),
        ("P_{FA}", "誤警報確率 [-]"),
        (r"\ln", "自然対数"),
        ...
        (r"\xi", "指数分布乱数（RCS変動を模擬）"),
        (r"\mathrm{Exp}(1)", "平均1の指数分布"),
        ("P_D", "パルス当たりの検出確率 [-]"),
    ])
    ...
    add_symbol_def(doc, [
        ...
        (r"\lambda_c", "クラッタ発生期待点数（ポアソン分布の平均）[点/フレーム]"),
        (r"\mathrm{Poisson}(\lambda_c)", "期待値 λ_c のポアソン分布"),
    ])
...
def add_section_2(doc):
    ...
    add_symbol_def(doc, [
        ...
        (r"\mathrm{diag}(\cdots)", "対角行列コンストラクタ"),
        (r"\sigma_r", "距離観測の標準偏差 [m]"),
        (r"\sigma_\theta", "方位角観測の標準偏差 [rad]"),
        (r"\sigma_\phi", "仰角観測の標準偏差 [rad]"),
        (r"\sigma_{\dot{d}}", "ドップラー観測の標準偏差 [m/s]"),
    ])
...
def add_section_3(doc):
    ...
    add_symbol_def(doc, [
        (r"\lambda", "UKFスケーリングパラメータ（= α²(n+κ)−n）"),
        (r"\alpha", "シグマポイント拡散パラメータ"),
        ("n", "状態次元数"),
        (r"\kappa", "二次スケーリングパラメータ"),
        (r"\beta", "分布パラメータ（高次モーメント補正）"),
    ])
    ...
    add_symbol_def(doc, [
        (r"\chi_0", "中心シグマポイント（= 状態推定値）"),
        (r"\chi_i \; (i=1,\ldots,n)", "正側シグマポイント"),
        (r"\chi_{i+n} \; (i=1,\ldots,n)", "負側シグマポイント"),
        (r"\bar{x}", "現在の状態推定値（平均）"),
        (r"\left[\sqrt{(n+\lambda)P}\right]_i", "行列平方根（コレスキー分解）のi列目"),
        ("P", "現在の推定誤差共分散行列"),
        ("n", "状態次元数（シグマポイント総数 = 2n+1）"),
        (r"\lambda", "スケーリングパラメータ"),
    ])
    ...
    add_symbol_def(doc, [
        ("W_0^m", "中心シグマポイントの平均用重み（負値になりうる）"),
        (r"W_i^m \; (i=1,\ldots,2n)", "正負シグマポイントの平均用重み = 1/(2(n+λ))"),
        ("W_0^c", "中心シグマポイントの共分散用重み = W_0^m + (1−α²+β)"),
        (r"W_i^c \; (i=1,\ldots,2n)", "正負シグマポイントの共分散用重み（= W_i^m と同値）"),
        (r"\beta", "分布パラメータ（高次モーメント補正）"),
        (r"\lambda", "スケーリングパラメータ（全重みの和は必ず 1）"),
        ("n", "状態次元数"),
    ])
    ...
    add_symbol_def(doc, [
        (r"\chi_i^*", "運動モデル適用後の伝播済みシグマポイント"),
        (r"f(\chi_i, \Delta t)", "運動モデル関数（CV / 弾道 / CT）"),
        (r"\Delta t", "タイムステップ [s]"),
        (r"\hat{x}^-", "予測状態推定値（更新前）"),
        ("W_i^m", "平均用重み係数"),
        ("P^-", "予測誤差共分散行列"),
        ("W_i^c", "共分散用重み係数"),
        ("Q", "連続時間プロセスノイズ強度行列"),
        (r"Q \cdot \Delta t", "離散時間プロセスノイズ共分散（Q は単位時間あたりの雑音強度）"),
        (r"(\cdot)^T", "行列転置"),
    ])
    ...
    add_symbol_def(doc, [
        ...
        (r"h(\chi_i^*)", "観測モデル関数（距離・方位・仰角・ドップラーに変換）"),
        (r"\hat{z}", "予測観測値（シグマポイントの重み付き平均）"),
        ("S", "イノベーション共分散行列"),
        ("R", "観測ノイズ共分散行列"),
    ...
    add_symbol_def(doc, [
        ...
        (r"\sigma_{pos}", "位置成分のプロセスノイズ標準偏差 [m]"),
        (r"\sigma_{vel}", "速度成分のプロセスノイズ標準偏差 [m/s]"),
        (r"\sigma_{acc}", "加速度成分のプロセスノイズ標準偏差 [m/s²]"),
        (r"\mathrm{diag}(\cdots)", "対角行列コンストラクタ（非対角要素 = 0）"),
    ])
...
def add_section_4(doc):
    ...
    add_symbol_def(doc, [
        (r"\mu_k", "モデル k の現在の確率（更新前）"),
        (r"\pi(k \to j)", "モデル k からモデル j への遷移確率"),
        ("c_j", "モデル j への予測確率（= Σ_k π(k→j)μ_k）"),
        (r"\mu_{k|j}", "モデル j を前提とした条件付き遷移確率"),
        (r"\tilde{x}_{0j}", "モデル j の UKF に入力する混合初期状態"),
        (r"\hat{x}", "IMM統合後の状態推定値"),
        (r"\hat{x}_j", "モデル j の UKF 更新後状態推定値"),
        ("P", "IMM統合後の誤差共分散行列"),
        ("P_j", "モデル j の誤差共分散行列"),
        ("L_j", "モデル j の尤度（イノベーションの正規分布確率密度）"),
        (r"\mathcal{N}(\nu; 0, S)", "平均0・共分散 S のガウス分布"),
        ("S_j", "モデル j のイノベーション共分散行列"),
        (r"\mu_j", "モデル j の更新後確率（規格化済み）"),
    ])
...
def add_section_5(doc):
    ...
    add_symbol_def(doc, [
        ...
        ("R_E", "地球半径（= 6,371,000 m）"),
        ("h", "目標の高度（= z）[m]"),
        (r"\rho(h)", "高度 h における大気密度 [kg/m³]"),
        (r"\rho_0", "海面大気密度（= 1.225 kg/m³）"),
        ("H", "大気スケール高度 [m]"),
        (r"\mathbf{a}_{drag}", "比抗力（単位質量あたりの抗力加速度）[m/s²]"),
        (r"\beta", "弾道係数（= C_d·A/(2m)）[m²/kg]"),
        (r"|\mathbf{v}|", "速度の大きさ [m/s]"),
        (r"\ddot{x}, \ddot{y}", "水平面内の加速度成分（抗力のみ）[m/s²]"),
        (r"\ddot{z}", "鉛直方向加速度（重力＋抗力）[m/s²]"),
    ])
...
def add_section_8(doc):
    ...
    add_symbol_def(doc, [
        ...
        ("N", "評価サンプル数（タイムステップ数）"),
        (r"\mathbf{p}_{track,i}", "タイムステップ i の追尾位置ベクトル [m]"),
        (r"\mathbf{p}_{truth,i}", "タイムステップ i の真値位置ベクトル [m]"),
        (r"\mathbf{v}_{track,i}", "タイムステップ i の追尾速度ベクトル [m/s]"),
        (r"\mathbf{v}_{truth,i}", "タイムステップ i の真値速度ベクトル [m/s]"),
        (r"\|\cdot\|", "ユークリッドノルム（3次元距離）"),
        (r"\sum_{i=1}^N", "i = 1 から N までの総和"),
    ])
    ...
    add_symbol_def(doc, [
        ...
        ("p", "ノルム次数"),
        ("c", "カットオフ距離（個数誤差のペナルティ上限）[m]"),
        ("m", "追尾集合 X の要素数（航跡数）"),
        ("n", "真値集合 Y の要素数（目標数）"),
        ("x_i, y_j", "集合 X, Y の各要素（位置ベクトル）"),
        (r"\pi(i)", "最適割当によるインデックスの対応付け"),
    ...
    add_symbol_def(doc, [
        ("TP", "True Positive：正しく追尾された目標数"),
        ("FP", "False Positive：誤追尾数（真値に対応しない余剰な航跡）"),
        ("FN", "False Negative：追尾漏れ数（未追尾の真値）"),
        (r"\mathrm{Precision}", "適合率（追尾航跡のうち正しいものの割合）"),
        (r"\mathrm{Recall}", "再現率（真値目標のうち追尾されたものの割合）"),
...
def add_section_gpu_acceleration(doc):
    ...
    add_symbol_def(doc, [
        ...
        ("2n+1", "シグマポイント総数（n: 状態次元数）"),
        (r"\text{blockDim}", "CUDAスレッドブロックサイズ"),
        (r"\text{Grid size}", "CUDAカーネルに渡すブロック数"),
        (r"N_{targets} \times 19", "1ステップ全シグマポイント総数"),
    ])
    ...
    add_symbol_def(doc, [
        ...
        (r"\mathbf{z}_j", "j 番目の観測値ベクトル"),
        (r"\hat{\mathbf{z}}_i", "航跡 i の予測観測値ベクトル"),
        ("S_i", "航跡 i のイノベーション共分散行列（4×4）"),
